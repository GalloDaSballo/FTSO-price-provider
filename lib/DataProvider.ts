...
class DataProvider {
  ...
  constructor(conf: any) {
    ...
    for (let ex of ["coinbasepro", "kraken"]) {
      exchanges.push({
        ex,
        market: "USDT/USD",
        client: this.getWsClient(ex, 3 * n),
      });
    }
    let usdtUsdProvider: IPriceProvider = new impl.WsLimitedPriceProvider(
      "USDT/USD",
      1.0,
      exchanges,
      "avg"
    ...
    this.data = conf.priceProviderList.map((ppc: any, index: number) => {
      ...
      let priceProvider: IPriceProvider = new (impl as any)[
        ppc.priceProviderClass
  ...
  getWsClient(ex: string, n: number): void {
    if (!this.ex2client[ex]) {
      ...
      client.on("error", (err: any) =>
        self.logger.error(`Error on exchange ${ex}: ${err}`)
      );
      client.on("reconnecting", () =>
        self.logger.info(`Reconnecting to ${ex}...`)
      );
      client.on("connecting", () => self.logger.info(`Connecting to ${ex}...`));
      client.on("connected", () => self.logger.info(`Connected to ${ex}...`));
      client.on("disconnected", () =>
        self.logger.info(`Disconnected from ${ex}...`)
      );
  ...
  async submitPriceHash(lst: DataProviderData[]) {
    ...
    let realEpochData = await this.ftsoManagerWeb3Contract.methods
    ...
    for (let p of lst) {
      ...
      if (price) {
        ...
        index2price.set(
          Number(this.symbol2Index.get(conf.symbolPrefix + p.symbol)),
          preparedPrice
        );
        this.logger.info(
    ...
    let ftsoIndices: number[] = [...index2price.keys()].sort(
      (a: number, b: number) => a - b
    );
    let prices: string[] = ftsoIndices.map((index: number) =>
    ...
    if (prices.length > 0) {
      ...
      var fnToEncode = this.priceSubmitterWeb3Contract.methods.submitHash(
        epochId,
        hash
      );
  ...
  async revealPrices(
    ...
    while (
      ...
      for (let p of lst) {
        ...
        let priceInfo = this.symbol2epochId2priceInfo
          .get(p.symbol)!
          .get(epochIdStr);
      ...
      if (prices.length > 0) {
        var fnToEncode = this.priceSubmitterWeb3Contract.methods.revealPrices(
          epochIdStr,
          ftsoIndices,
          prices,
          random
  ...
  async setupSubmissionAndReveal() {
    ...
    let epochSubmitTimeEnd: number = this.epochSettings
      .getEpochSubmitTimeEnd()
      .toNumber();
    let epochRevealTimeEnd: number = this.epochSettings
      .getEpochReveaTimeEnd()
      .toNumber();
    ...
    if (
      ...
      setTimeout(() => {
        this.logger.info(
          `Submit in ${diffSubmit - submitPeriod + conf.submitOffset}ms`
        );
        this.execute(async () => {
    ...
    this.priceSubmitterContract.on(
      ...
      async (
        ...
        for (let ftso of ftsos) {
          ...
          let p: DataProviderData = this.symbol2dpd.get(
            symbol.replace(new RegExp(`^(${conf.symbolPrefix})`), "")
          )!;
          let price = prices[i];
          this.logger.info(
          ...
          if (priceInfo) {
            ...
            if (p) {
              ...
              if (priceInfo.priceSubmitted != (price as number)) {
                this.logger.error(
                  `${p.label} | Price submitted and price revealed are diffent!`
                );
              }
            }
          }
  ...
  async runDataProvider() {
    ...
    if (process.env.PROJECT_SECRET === undefined) {
      ...
      accountPrivateKey = process.env.PROJECT_SECRET as string; // NOTE: We grab the PK from .env
    } else {
      this.logger.info(
        `Starting Flare Price Provider  v${version} [developer mode]`
      );
    ...
    if (process.env.RPC_URL !== undefined) {
      conf.rpcUrl = process.env.RPC_URL; /// Note: We pass the RPC through here
    ...
    try {
      let ftsoRegistryAddress = await this.priceSubmitterWeb3Contract.methods
        .getFtsoRegistry()
        .call();
      ...
      try {
        ...
        for (let ftso of lst) {
          ...
          try {
            ...
            let index = await this.ftsoRegistryContract.methods
              .getFtsoIndex(symbol)
              .call();
    ...
    try {
      let data = (await this.ftsoManagerWeb3Contract.methods
